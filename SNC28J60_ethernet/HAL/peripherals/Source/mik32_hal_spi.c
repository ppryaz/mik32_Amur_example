...
__attribute__((weak)) void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
    ...
    if (hspi->Instance == SPI_0)
    {
        ...
        {
        case SPI_CS_0:
            GPIO_InitStruct.Pin = PORT0_4; //Было POrt0_4
            HAL_GPIO_Init(GPIO_0, &GPIO_InitStruct);
            break;

        case SPI_CS_1:
            GPIO_InitStruct.Pin = PORT0_14;
            HAL_GPIO_Init(GPIO_1, &GPIO_InitStruct);
            break;

        case SPI_CS_2:
            GPIO_InitStruct.Pin = PORT0_15;
            HAL_GPIO_Init(GPIO_1, &GPIO_InitStruct);
            break;

        case SPI_CS_3:
            GPIO_InitStruct.Pin = PORT0_6;
...
void HAL_SPI_Wait(SPI_HandleTypeDef *hspi, uint8_t resp[]) {
	uint32_t timeout = 0;
	uint8_t data;
	//uint8_t resp;

	/*  do {
	        HAL_SPI_Exchange(&hspi, &data, resp, 1, SPI_TIMEOUT_DEFAULT);
	        timeout += 1;
