...
// set CS to 0 = active
#define CSACTIVE HAL_SPI_CS_Enable(&hspi1, SPI_CS_0)
// set CS to 1 = passive
#define CSPASSIVE HAL_SPI_CS_Disable(&hspi1)

// max frame length which the controller will accept:
#define MAX_FRAMELEN	(NET_BUFFER_SIZE+4)        // maximum ethernet frame length

#define MAX_TX_BUFFER	NET_BUFFER_SIZE //NET_BUFFER_SIZE
#define MAX_RX_BUFFER	NET_BUFFER_SIZE //NET_BUFFER_SIZE
// start with recbuf at 0/
#define RXSTART_INIT	0x0000
#define RXSTOP_INIT		(8192-NET_BUFFER_SIZE-1) //NET_BUFFER_SIZE
#define TXSTART_INIT	(8192-NET_BUFFER_SIZE) //NET_BUFFER_SIZE
...
unsigned short enc28j60_tx_checksum(uint16_t offset, uint16_t len );

uint16_t enc28j60_packet_receive (uint8_t *rxtx_buffer, uint16_t max_length );
void enc28j60_packet_send (uint8_t *buffer, uint16_t length);


uint8_t enc28j60_SPI_ReadWrite(uint8_t  data);
void snc28j60_SPI_SendByte(uint8_t bt);
uint8_t snc28j60_SPI_ReceiveByte();
