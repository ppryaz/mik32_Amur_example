...
// Настройки
#define W5500_MODE_DEBUG
#define W5500_DHCP_SOCK			0
#define W5500_DHCP_RETRY_MAX	3

#define W5500_SERVER_TCP_SOCK	1
#define W5500_SERVER_TCP_PORT	5577

#define W5500_SERVER_UDP_SOCK	2
#define W5500_SERVER_UDP_PORT	5578

#define W5500_DATA_BUF_SIZE		2048
#define W5500_SPI_TIMEOUT		5000

// Сохранение настроек в EEPROM
//#define W5500_EEPROM_USE
#define W5500_EEPROM_DEVICE_TYPE	AT24C16
#define W5500_EEPROM_DEVICE_ADDRESS	Adr_AT24Cxx
#define W5500_EEPROM_MEMORY_ADDRESS	0x00

////////////////////////////////////////////////////////////////////////////////////////////////////

typedef void (*W5500_ServerTCP_OnReceive)(uint8_t* pData, uint16_t* pDataSize, uint16_t pDataSizeMax);									// Обработчик события Приема данных от Клиента Сервера TCP. Передаются указатель на Полученные данные и их Размер, а возвращаются заполненные ответом
typedef void (*W5500_ServerUDP_OnReceive)(uint8_t* pData, uint16_t* pDataSize, uint16_t pDataSizeMax, uint8_t* addr, uint16_t port);	// Обработчик события Приема данных от Клиента Сервера UDP. Передаются указатель на Полученные данные, их Размер и Адрес+Порт Клиента, а возвращаются заполненные ответом

////////////////////////////////////////////////////////////////////////////////////////////////////

uint8_t W5500_Start();												// Инициализация микросхемы
int32_t W5500_ServerTCP_Step(W5500_ServerTCP_OnReceive onReceive);	// TCP Сервер (Функция вызывается или в Таймере или Цикле)
int32_t W5500_ServerUDP_Step(W5500_ServerUDP_OnReceive onReceive);	// UDP Сервер (Можно использовать для поиска по Broadcast) (Функция вызывается или в Таймере или Цикле)
